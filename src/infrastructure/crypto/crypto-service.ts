...
export class CryptoService {
  ...
  async generateKeyPair(): Promise<{
    ...
    const keyPair = await crypto.subtle.generateKey(
      ...
      true, // Keys MUST be extractable for JWK export
      ["sign", "verify"],
    );
  ...
  async encrypt(data: string, masterKey: string): Promise<string> {
    ...
    // SECURITY: keyMaterial is non-extractable (PBKDF2 key), cannot export.
    // The derived aesKey is also non-extractable and used only for encrypt/decrypt.
    // Cleanup happens implicitly when the CryptoKey objects go out of scope.
